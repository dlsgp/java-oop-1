...
class A {   // 바깥 클래스 A
    ...
    static class B{ // static 내부 클래스
        ...
        void method1(){
            System.out.println("static 내부 클래스의 mehtod1()");
        }
    ...
    void method(){  // 바깥 클래스의 메서드
        class D{    // 로컬 내부 클래스
            ...
            void method1(){
                System.out.println("로컬 내부 클래스의 mehtod1()");
            }
        }
    ...
    /*
    A 클래스는 B와 C 두개의 내부 클래스를 가지고 있다.
    B 클래스는 static 내부클래스, C 클래스는 인스턴스 내부 클래스 입니다.
    static 내부 클래스는 바깥 클래스의 인스턴스와 상관없이 독립적으로 존재합니다.
    인스턴스 내부 클래스는 바깥 클래스의 인스턴스를 통해서 생성됩니다.
    각 각의 내부 클래스는 자체적인 메서드와 변수를 가질 수 있습니다.
    static 메서드는 인스턴스를 생성하지 않고 생성할 수 있다.
     */
